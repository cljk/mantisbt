 ...
 */
#000:function file_add( $p_bug_id, $p_file, $p_table = 'bug', $p_title = '', $p_desc = '', $p_user_id = null ) {
...
function file_get_content( $p_file_id, $p_type = 'bug' ) {
	...
	switch ( $p_type ) {
		case 'bug':
			#001: $t_bug_file_table = db_get_table( 'bug_file' );
			$t_bug_file_table = db_get_table( 'mantis_bug_file_table' );
			$query = "SELECT *
				FROM $t_bug_file_table
				WHERE id=" . db_param();
			break;
		case 'doc':
			#001: $t_project_file_table = db_get_table( 'project_file' );
			$t_project_file_table = db_get_table( 'mantis_project_file_table' );
			$query = "SELECT *
				FROM $t_project_file_table
				WHERE id=" . db_param();
